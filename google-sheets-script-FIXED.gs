...
function initializeSpreadsheet() {
	...
	// Format the header
	responsesSheet.getRange(1, 1, 1, headers.length).setBackground('#2ECC71').setFontColor('#FFFFFF');
...
function createAnalyticsSheet(ss) {
	...
	const dashboard = [
		...
		[
			'=IFERROR(INDEX(SORT(UNIQUE(FILTER(Responses!M:M,Responses!M:M<>"Quartier/Zone",Responses!M:M<>"",Responses!M:M<>"Diaspora")),1,TRUE),1),"N/A")',
			'=IFERROR(COUNTIF(Responses!M:M,A14),0)',
			'',
			'',
		],
		[
			'=IFERROR(INDEX(SORT(UNIQUE(FILTER(Responses!M:M,Responses!M:M<>"Quartier/Zone",Responses!M:M<>"",Responses!M:M<>"Diaspora")),1,TRUE),2),"N/A")',
			'=IFERROR(COUNTIF(Responses!M:M,A15),0)',
			'',
			'',
		],
		[
			'=IFERROR(INDEX(SORT(UNIQUE(FILTER(Responses!M:M,Responses!M:M<>"Quartier/Zone",Responses!M:M<>"",Responses!M:M<>"Diaspora")),1,TRUE),3),"N/A")',
			'=IFERROR(COUNTIF(Responses!M:M,A16),0)',
			'',
			'',
		],
		[
			'=IFERROR(INDEX(SORT(UNIQUE(FILTER(Responses!M:M,Responses!M:M<>"Quartier/Zone",Responses!M:M<>"",Responses!M:M<>"Diaspora")),1,TRUE),4),"N/A")',
			'=IFERROR(COUNTIF(Responses!M:M,A17),0)',
			'',
			'',
		],
		[
			'=IFERROR(INDEX(SORT(UNIQUE(FILTER(Responses!M:M,Responses!M:M<>"Quartier/Zone",Responses!M:M<>"",Responses!M:M<>"Diaspora")),1,TRUE),5),"N/A")',
			'=IFERROR(COUNTIF(Responses!M:M,A18),0)',
			'',
			'',
		],
		['', '', '', ''],
		...
		[
			'=TODAY()-6',
			'=COUNTIFS(Responses!A:A,">="&A22,Responses!A:A,"<"&A22+1)',
			'=COUNTIFS(Responses!A:A,">="&A22,Responses!A:A,"<"&A22+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A22,Responses!A:A,"<"&A22+1,Responses!L:L,"etranger")',
		],
		[
			'=TODAY()-5',
			'=COUNTIFS(Responses!A:A,">="&A23,Responses!A:A,"<"&A23+1)',
			'=COUNTIFS(Responses!A:A,">="&A23,Responses!A:A,"<"&A23+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A23,Responses!A:A,"<"&A23+1,Responses!L:L,"etranger")',
		],
		[
			'=TODAY()-4',
			'=COUNTIFS(Responses!A:A,">="&A24,Responses!A:A,"<"&A24+1)',
			'=COUNTIFS(Responses!A:A,">="&A24,Responses!A:A,"<"&A24+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A24,Responses!A:A,"<"&A24+1,Responses!L:L,"etranger")',
		],
		[
			'=TODAY()-3',
			'=COUNTIFS(Responses!A:A,">="&A25,Responses!A:A,"<"&A25+1)',
			'=COUNTIFS(Responses!A:A,">="&A25,Responses!A:A,"<"&A25+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A25,Responses!A:A,"<"&A25+1,Responses!L:L,"etranger")',
		],
		[
			'=TODAY()-2',
			'=COUNTIFS(Responses!A:A,">="&A26,Responses!A:A,"<"&A26+1)',
			'=COUNTIFS(Responses!A:A,">="&A26,Responses!A:A,"<"&A26+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A26,Responses!A:A,"<"&A26+1,Responses!L:L,"etranger")',
		],
		[
			'=TODAY()-1',
			'=COUNTIFS(Responses!A:A,">="&A27,Responses!A:A,"<"&A27+1)',
			'=COUNTIFS(Responses!A:A,">="&A27,Responses!A:A,"<"&A27+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A27,Responses!A:A,"<"&A27+1,Responses!L:L,"etranger")',
		],
		[
			'=TODAY()',
			'=COUNTIFS(Responses!A:A,">="&A28,Responses!A:A,"<"&A28+1)',
			'=COUNTIFS(Responses!A:A,">="&A28,Responses!A:A,"<"&A28+1,Responses!C:C,">=70")',
			'=COUNTIFS(Responses!A:A,">="&A28,Responses!A:A,"<"&A28+1,Responses!L:L,"etranger")',
		],
	];
...
function createHotLeadsSheet(ss) {
	...
	sheet
		.getRange('A3')
		.setFormula('=IFERROR(FILTER(Responses!A:BB, Responses!C:C>=70, ROW(Responses!A:A)>1), "Aucun hot lead trouvé")');

	// Format header row
	sheet.getRange(3, 1, 1, 54).setBackground('#E74C3C').setFontColor('#FFFFFF').setFontWeight('bold');
}
...
function createDiasporaSheet(ss) {
	...
	// Format header row
	sheet.getRange(3, 1, 1, 54).setBackground('#3498DB').setFontColor('#FFFFFF').setFontWeight('bold');
}
...
function calculateLeadScore(data) {
	...
	if (highFreq.includes(data.frequence_usage || data.freq_utilisation_diaspora)) {
		...
		(data.frequence_usage || data.freq_utilisation_diaspora) === '1-2fois' ||
		(data.frequence_usage || data.freq_utilisation_diaspora) === 'une_fois_semaine'
	) {
		score += 8;
	}
...
/**
 * Check for duplicate submissions with improved phone format handling
 */
function checkDuplicate(sheet, phone, email) {
	...
	// Search for columns with flexible matching
	for (let i = 0; i < headers.length; i++) {
		...
		if (header.includes('promo') || header.includes('code')) {
			// Make sure we get the actual promo code column, not status/value columns
			if (!header.includes('statut') && !header.includes('valeur') && !header.includes('expir') && !header.includes('utilis')) {
				promoCol = i;
			}
		}
	}

	// Fallback to exact matches if flexible search didn't work
	if (phoneCol === -1) phoneCol = headers.indexOf('Téléphone');
	if (emailCol === -1) emailCol = headers.indexOf('Email');
	if (timestampCol === -1) timestampCol = headers.indexOf('Date de soumission');
	if (promoCol === -1) promoCol = headers.indexOf('Code Promo');
	...
	for (let i = 1; i < values.length; i++) {
		...
		// Clean the row phone number (keep only digits)
		const cleanRowPhone = rowPhone.toString().replace(/\D/g, '');

		// Compare the last 9 digits (Senegalese number without country code)
		// This handles: 771234567 vs +221771234567 vs 00221771234567
		const phoneEnd = cleanPhone.slice(-9);
		const rowPhoneEnd = cleanRowPhone.slice(-9);

		// Log comparison for debugging
		if (phoneEnd && rowPhoneEnd) {
			console.log(`Row ${i+1}: Comparing ${phoneEnd} with ${rowPhoneEnd}`);
		}
	...
	console.log('No duplicate found for phone:', cleanPhone, 'or email:', email);
	return null;
}
...
/**
 * Test function to verify duplicate detection
...
function testDuplicateDetection() {
	...
	const testCases = [
		{ phone: '771234567', email: null, description: 'Local format' },
		{ phone: '+221771234567', email: null, description: 'With +221' },
		{ phone: '00221771234567', email: null, description: 'With 00221' },
		{ phone: '77 123 45 67', email: null, description: 'With spaces' },
		{ phone: '+221 77 123 45 67', email: null, description: '+221 with spaces' },
		{ phone: '(77) 123-45-67', email: null, description: 'With parentheses and dashes' },
	];
	
	// Get actual phone numbers from the sheet
	if (sheet.getLastRow() > 1) {
		...
		if (phoneCol >= 0 && values[1][phoneCol]) {
			const existingPhone = values[1][phoneCol].toString();
			console.log('\nExisting phone in row 2:', existingPhone);
			
			// Extract last 9 digits for testing
			const cleanExisting = existingPhone.replace(/\D/g, '');
			const last9 = cleanExisting.slice(-9);
			
			console.log('Cleaned:', cleanExisting);
			console.log('Last 9 digits:', last9);
			
			// Test with the actual number in different formats
			const actualTests = [
				{ phone: last9, description: 'Just 9 digits' },
				{ phone: '+221' + last9, description: 'With +221' },
				{ phone: '00221' + last9, description: 'With 00221' },
			];
			
			console.log('\n--- Testing with actual number from sheet ---');
			for (const test of actualTests) {
				const result = checkDuplicate(sheet, test.phone, null);
				console.log(`Test "${test.description}" (${test.phone}):`, 
					result ? `✅ FOUND duplicate in row ${result.existingRow}` : '❌ No duplicate found');
			}
		}
	}
	...
	for (const test of testCases) {
		const result = checkDuplicate(sheet, test.phone, test.email);
		console.log(`Test "${test.description}" (${test.phone}):`, 
			result ? `Found duplicate in row ${result.existingRow}` : 'No duplicate');
	}
	
	console.log('\n===== TEST COMPLETE =====');
	console.log('Check the Execution log for results');
}
